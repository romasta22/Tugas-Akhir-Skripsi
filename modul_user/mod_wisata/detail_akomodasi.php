<?php
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id_wisata'])) {
            ...
            if ($result->num_rows == 0) {
                ...
                echo "<script>alert('Wisata sudah ada di favorit Anda.'); window.location.href = 'index_user.php?hal=detail_akomodasi&id=$id_wisata'; </script>";
            }
            // Tutup koneksi
            $stmt->close();
            $conn->close();
        ...
        echo "<script type='text/javascript'>alert('Ulasan berhasil dikirim!');window.location.href = 'dashboard_user.php?hal=detail_akomodasi&id=" . $id_wisata . "';</script>";
...
.star-rating label:hover ~ label {
    color: #ffcc00; /* Hover effect untuk semua bintang sebelumnya */
        ...
        <h2>Rekomendasi Wisata  Akomodasi Serupa</h2>
      ...
      <!-- Tombol Kembali -->
      <div class="mt-4">
            <a href="dashboard_user.php?hal=akomodasi" class="btn btn-secondary">Kembali ke Daftar Akomodasi</a>
        </div>
    </div>  
    <script>
        ...
        document.getElementById('btnShare').addEventListener('click', function(event) {
            ...
            if (navigator.share) {
                navigator.share({
                    title: 'Detail Wisata Akomodasi',
                    text: 'Lihat detail wisata akomodasi ini!',
