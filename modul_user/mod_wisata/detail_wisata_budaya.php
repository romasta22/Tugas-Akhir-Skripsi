<?php
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id_wisata'])) {
            ...
            if ($result->num_rows == 0) {
                ...
                echo "<script>alert('Wisata sudah ada di favorit Anda.'); window.location.href = 'index_user.php?hal=detail_wisata_budaya&id=$id_wisata'; </script>";
            }
            // Tutup koneksi
            $stmt->close();
            $conn->close();
...
echo "<script type='text/javascript'>alert('Ulasan berhasil dikirim!');window.location.href = 'dashboard_user.php?hal=detail_wisata_budaya&id=" . $id_wisata . "';</script>";
...
<head>
    ...
    <title>Detail Wisata Budaya</title>
...
<style>
    ...
    .star-rating label:hover ~ label {
        ...
        <h2>Rekomendasi Wisata  Budaya Serupa</h2>
      ...
      <!-- Tombol Kembali -->
      <div class="mt-4">
            <a href="dashboard_user.php?hal=wisata_budaya" class="btn btn-secondary">Kembali ke Daftar Wisata Budaya</a>
        </div>
    </div>  
    <script>
        ...
        document.getElementById('btnShare').addEventListener('click', function(event) {
            ...
            if (navigator.share) {
                navigator.share({
                    title: 'Detail Wisata budaya',
                    text: 'Lihat detail wisata budaya ini!',
