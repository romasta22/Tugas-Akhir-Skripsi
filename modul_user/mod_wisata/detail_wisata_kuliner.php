<?php
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id_wisata'])) {
            ...
            if ($result->num_rows == 0) {
                ...
                echo "<script>alert('Wisata sudah ada di favorit Anda.'); window.location.href = 'index_user.php?hal=detail_wisata_kuliner&id=$id_wisata'; </script>";
            }
            // Tutup koneksi
            $stmt->close();
            $conn->close();
...
echo "<script type='text/javascript'>alert('Ulasan berhasil dikirim!');window.location.href = 'dashboard_user.php?hal=detail_wisata_kuliner&id=" . $id_wisata . "';</script>";
...
<head>
    ...
    <title>Detail Wisata Kuliner</title>
...
.star-rating label:hover ~ label {
    color: #ffcc00; /* Hover effect untuk semua bintang sebelumnya */
        ...
        <h2>Rekomendasi Wisata  Kuliner Serupa</h2>
      ...
      <!-- Tombol Kembali -->
      <div class="mt-4">
            <a href="dashboard_user.php?hal=wisata_kuliner" class="btn btn-secondary">Kembali ke Daftar Wisata Kuliner</a>
        </div>
    </div>  
    <script>
        ...
        document.getElementById('btnShare').addEventListener('click', function(event) {
            ...
            if (navigator.share) {
                navigator.share({
                    title: 'Detail Wisata Kuliner',
                    text: 'Lihat detail wisata kuliner ini!',
